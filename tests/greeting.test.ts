...
describe("greeting", () => {
  test("names the month and what is left of it", () => {
    ...
    const { summary, question } = greeting(salah, new Date(2026, 8, 20, 9, 30));
  ...
  test("says nothing about a daily allowance", () => {
    assert.doesNotMatch(greeting(salah, new Date(2026, 8, 20)).summary, /day\)|\/day/);
  });

  test("the last day of the month still has a day to go", () => {
    const { summary } = greeting(salah, new Date(2026, 8, 30, 23, 0));
  ...
  test("the first of the month counts the whole month", () => {
    const { summary } = greeting(salah, new Date(2026, 0, 1));
    assert.match(summary, /left for January, 31 days to go/);
  });

  test("February knows about leap years", () => {
    assert.match(greeting(salah, new Date(2028, 1, 1)).summary, /29 days to go/);
    assert.match(greeting(salah, new Date(2027, 1, 1)).summary, /28 days to go/);
  });

  test("a currency with a symbol reads as a symbol", () => {
    const withUsd = createProfile("Salah", 4000, "USD");
    assert.match(greeting(withUsd, new Date(2026, 8, 20)).summary, /\$4,000 left for September/);
  });
});
